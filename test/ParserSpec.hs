...
spec = do
  ...
  describe "Task Parsing (TEST_SPEC.md#1.2)" $ do
    ...
    it "parses task with UID comment" $ do
      -- User story: "The system adds invisible UIDs for sync tracking"
      let input = ". Task <!-- UID:550e8400e29b41d4 -->"
      case parseTaskLine (fromGregorian 2025 8 16) input of
        Right task -> taskUid task `shouldBe` Just "550e8400e29b41d4"
        Left err -> expectationFailure $ "Parse failed: " ++ show err
  ...
  describe "Context Extraction (TEST_SPEC.md#1.4)" $ do
    ...
    it "extracts multiple contexts" $ do
      extractContexts "Task @home @urgent @computer" `shouldBe` 
        [Context "home", Context "urgent", Context "computer"]
    
    it "handles contexts with hyphens and underscores" $ do
      extractContexts "Task @home-office @high_priority" `shouldBe`
        [Context "home-office", Context "high_priority"]
