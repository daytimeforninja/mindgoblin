            ...
            case T.splitOn "-" t of
                [start, end] ->
                    ...
                    let suffix = T.takeWhileEnd (`elem` ("ampmAMPM" :: String)) end
                        startWithSuffix =
                            if T.null suffix || any (`T.isSuffixOf` start) ["am", "pm", "AM", "PM"]
                                then start
                                else start <> suffix
    ...
    parseTimeToISO t
        | "pm" `T.isSuffixOf` T.toLower t =
            let hourStr = T.dropEnd 2 t
                hour = case readMaybe (T.unpack hourStr) :: Maybe Int of
                    Just h -> if h < 12 then h + 12 else h
                    Nothing -> 14 -- Default to 2pm
             in T.pack $ printf "%02d0000" hour
        | "am" `T.isSuffixOf` T.toLower t =
            let hourStr = T.dropEnd 2 t
                hour = case readMaybe (T.unpack hourStr) :: Maybe Int of
                    Just h -> if h == 12 then 0 else h
                    Nothing -> 9 -- Default to 9am
             in T.pack $ printf "%02d0000" hour
        | ":" `T.isInfixOf` t =
            let parts = T.splitOn ":" t
             in case parts of
                    [h, m] -> case (readMaybe (T.unpack h) :: Maybe Int, readMaybe (T.unpack m) :: Maybe Int) of
                        (Just hInt, Just mInt) -> T.pack (printf "%02d%02d00" hInt mInt)
                        _ -> "140000" -- Default if parsing fails
                    _ -> "140000" -- Default
         ...
         in case parseTimeRange timeStr of
                ...
                Nothing ->
                    ...
                    let startTimeISO = parseTimeToISO timeStr
                        ...
                        hour = case readMaybe (T.unpack hourStr) :: Maybe Int of
                            Just h -> if h < 23 then h + 1 else 0
                            Nothing -> 15
